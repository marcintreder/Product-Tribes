...
$("input[type=text]").blur(function(element){
  ...
  //ON BLUR FORM VALIDATION. Not reusable because of hard-coded ID names.
  $("input[type=text]").blur(function (){
    ...
    //removes messages when there's a correct data in the "job title" field
    if (inputId == "#job-title" && inputValue.length > 0){
...
//GOOGLE SPREADSHEET SENDED SCRIPT.It's a modified script from: https://github.com/dwyl/web-form-to-google-sheet. I've added "on submit" validation.
    	...
    	$("#apply").submit(function(event){
          ...
          // success message
          $('.wait').remove();
          $('#submit-button').empty().removeClass('primary-button').addClass('success').html('Application received. Expect reply in 72 hours.');
